  ...
  describe('POST /api/users', function () {
    ...
    it('Should reject users with non-string password', function() {
      return chai
        .request(app)
        .post('/api/users')
        .send({username, password: 1234567890, fullname})
        .then(res => {
          expect(res).to.have.status(422);
    ...
    it('Should reject users with non-trimmed username', function() {
      return chai
        .request(app)
        .post('/api/users')
        .send({username: ' nontrimmeduser', password, fullname})
        .then(res => {
          expect(res).to.have.status(422);
          expect(res.body.message).to.equal('Your username must not have any spaces'); 
        });
    });
    ...
    it('Should reject users with non-trimmed password', function() {
      return chai
        .request(app)
        .post('/api/users')
        .send({username, password:  ' nontrimmedpassword', fullname})
        .then(res => {
          expect(res).to.have.status(422);
          expect(res.body.message).to.equal('Your password must not have any spaces'); 
        });
    });
    ...
    it('Should reject users with password less than 8 characters', function() {
      return chai
        .request(app)
        .post('/api/users')
        .send({username, password: 'hi', fullname})
        .then(res => {
          expect(res).to.have.status(422);
    ...
    it('Should reject users with password greater than 72 characters', function() {
      return chai
        .request(app)
        .post('/api/users')
        .send({username, password: '1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890', fullname})
        .then(res => {
          expect(res).to.have.status(422);
    ...
    it('Should trim fullname', function() {
      return chai
        .request(app)
        .post('/api/users')
        .send({username, password, fullname: ' Lavender Brown '})
        .then(res => {
          expect(res.body.fullname).to.equal('Lavender Brown');
        });
    });
