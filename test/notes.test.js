    ...
    it('should create and return when folderId is an empty string', function () {
      ...
      return chai.request(app)
        ...
        .then(function (_res) {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body).to.include.keys('id', 'title', 'userId', 'createdAt', 'updatedAt', 'tags');
          return Note.findOne({ _id: res.body.id });
        })
        .then(data => {
          expect(res.body.id).to.equal(data.id);
    ...
    it('should update the note when provided a valid tag', function () {
      ...
      return Promise.all([
        ...
        .then(function (res) {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body).to.include.keys('id', 'title', 'userId', 'content', 'tags', 'createdAt', 'updatedAt');
          expect(res.body.id).to.equal(data.id);
          expect(res.body.title).to.equal(data.title);
          expect(res.body.content).to.equal(data.content);
          expect(res.body.userId).to.equal(data.userId.toString());
